...
(async () => {
  ...
  if (stackRepos) {
    ...
    const workspaceReposDir = isLocal
      ? path.join(os.homedir(), ".furnace", "repo")
      : "";
    ...
    for (const repo of stackRepos) {
      ...
      const currentRepoCacheDir = path.join(
        reposCacheDir,
        repo.bits.org,
        repo.bits.repo
      );
      ...
      if (workspaceReposDir !== "" && fsUtils.exists(workspaceReposDir)) {
        const workspaceRepoPath = path.join(
          workspaceReposDir,
          repo.bits.org,
          repo.bits.repo
        );
  ...
  console.log(
    `deploying stack '${config.stack.name}' in env '${environment}' for platform '${platform}'`
  );
